...
/**
 * Offer wizard container managing navigation between steps
 */
export default function OfferWizardContainer({
  route,
  navigation,
}: RootStackScreenProps<'OfferWizard'>) {
  const { draftId, inResponseTo } = route.params || {};
  ...
  const createOffer = useMessageStore((state) => state.createOffer);
  ...
  useEffect(() => {
    if (!currentDraftId) {
      const newDraftId = createDraft('offer', inResponseTo);
  ...
  }, [currentDraftId, inResponseTo, createDraft, getDraft]);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      await createOffer({
        ...
        note: draft.note,
        inResponseTo: draft.inResponseTo,
      });
  ...
  }, [draft, currentDraftId, createOffer, deleteDraft, navigation]);
